            ...
            <div class="card">
                ...
                <div class="row">
                    <div class="col-12 text-right">
                        <a href="{{ route('data.create') }}" class="btn btn-sm btn-success">{{ __('Добавить новый') }}</a>
                    </div>
                </div>
                <div class="card-body">
                    <table class="table">
                        ...
                        @forelse($dataRegion as $item)
                            <tr>
                                ...
                                <td class="text-primary text-center">
                                    <a href="{{ route('data.show', ['dataCollection' => $item]) }}" type="button" rel="tooltip" title="Предварительный просмотр" class="btn btn-warning btn-link btn-sm">
                                        <i class="material-icons">visibility</i>
                                    </a>
                                    @if($item->user_id == Auth::user()->id)
                                    <a href="{{ route('data.edit', ['dataCollection' => $item]) }}" type="button" rel="tooltip" title="Редактировать" class="btn btn-primary btn-link btn-sm">
                                    ...
                                    @can('ruhsat berish')
                                        <ul class="list-inline">
                                            <li class="list-inline-item">
                                                <form action="{{ route('data.confirm', ['dataCollection' => $item]) }}" method="post">
                                                    @csrf
