...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{ route('data-poor-population.submit', [$table]) }}" autocomplete="off" class="form-horizontal">
            @csrf
            @method('post')
                          ...
                          <div class="form-group{{ $errors->has('indicator_id') ? ' has-danger' : '' }}">
                              <select class="form-control{{ $errors->has('indicator_id') ? ' is-invalid' : '' }}" name="indicator_id" id="input-indicator_id" required >
                                  ...
                                  @foreach($indicators as $indicator)
                                      <option value="{{ $indicator ->id}}" {{ $indicator->hasParent?'disabled':'' }}>{{ $indicator->name_uz }}</option>
                                  @endforeach
                              </select>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('value') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('value') ? ' is-invalid' : '' }}" name="value" id="input-name" type="text" placeholder="{{ __('Value') }}" value="{{ old('value') }}" required="true" aria-required="true"/>
                      @if ($errors->has('value'))
